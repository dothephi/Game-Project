...
public class WordChecker : MonoBehaviour
{
    ...
    private Ray SelectRay(Vector2 firstPosition, Vector2 secondPosition)
    {
        ...
        if (Math.Abs(direction.x + 1f) < tolerance && Math.Abs(direction.y - 1f) < tolerance)
        {
    ...
    private void CheckBoardCompleted()
    {
        ...
        if(currentGameData.selectedBoardData.SearchWords.Count == _completedWords)
        {
            ...
            var currentBoardIndex = DataSaver.ReadCategoryCurrentIndexValue(categoryName);
            ...
            for (int index = 0; index < gameLevelData.data.Count; index++)
            {
                ...
                {
                    nextBoardIndex = DataSaver.ReadCategoryCurrentIndexValue(gameLevelData.data[index].categoryName);
            ...
            if (currentBoardIndex < currentLevelSize) currentBoardIndex += 1;
