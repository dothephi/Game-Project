...
public class SearchingWordList : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (_wordsNumber < _columns) _rows = 1;
    ...
    private bool ShouldScaleDown(Vector3 targetScale) {
        ...
        var totalSqauresHeight = squareSize.y * _rows;

        if(totalSqauresHeight > parentRect.rect.height)
        {
            while(totalSqauresHeight > parentRect.rect.height)
            {
                if (TryIncreaseColumnNumber()) totalSqauresHeight = squareSize.y * _rows;
                else 
                    return true;
            }
        }

        var totalSquareWith = squareSize.x * _columns;

        if (totalSquareWith > parentRect.rect.width) return true;
